...
animal %>% select(-year,-Total,-outcome) %>%
  ...
  ggtitle("Animal Values' Variation Trends in Different States")+
  xlab("Animal Type") +
  ylab("Value") 
  ```
...
animal %>%select(-(4:11)) %>% ggplot(aes(x=year,y=Total,col=outcome)) +
  ...
  ggtitle("Different States' Animal Values in Different Years")+
  xlab("Year") +
  ylab("Total Value") 
```
```{r}
animal %>% group_by(animal_type,year)%>%summarise(to=sum(Total)) %>% ggplot(aes(x=year,y=to)) +
  ...
  facet_wrap(~animal_type,scales = 'free_y') +
  facet_wrap(~animal_type,scales = 'free_y')+
  ggtitle("Number of Animals Varies by Years")+
  xlab("Year") +
  ylab("Total Values") 
```
```{r}
